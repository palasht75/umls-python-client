...
def convert_to_rdf(json_data: dict, namespace_url: str = "http://example.org/umls#") -> str:
    """
...
def add_triples_from_dict(g: Graph, data: dict, UMLS: Namespace):
    ...
    common_fields = [
        "ui", "name", "rootSource", "atomCount", "obsolete", "suppressible",
        "attributes", "atoms", "ancestors", "parents", "children", 
        "descendants", "relations", "defaultPreferredAtom", "definitions"
    ]
    ...
    for field in common_fields:
        ...
        if field_value and field_value != "NONE":  # Skip None or "NONE" values
            if isinstance(field_value, str) and field_value.startswith("http"):  # If it's a URL
